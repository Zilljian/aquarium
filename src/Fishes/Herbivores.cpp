...
Herbivores::Herbivores(int x, int y, Field* initField) : Creature(9, 30, 5, x, y, "Herbivores") {
    field = initField;
}
...
Creature* Herbivores::breedDescendant() {
    ...
    return new Herbivores(xPos + xShift, yPos + yShift, field);
}

Creature* Herbivores::getClosest() {
...
Creature* Herbivores::checkField(int initShift, int maxShift) {
